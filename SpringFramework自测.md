...
> 
>
> <font color = "red">总结</font>：XA 模式优点为事务强一致性，常用数据库都实现了XA模式，在使用时没有代码侵入。缺点为在一阶段需要锁定数据库资源，等待二阶段结束才释放，降低了并发度，且依赖关系型数据库自己的事务机制。
...
>
> <font color = "red">总结</font>：2PC是一种尽量保证强一致性的分布式事务，也是**同步阻塞**的，同步阻塞会造成事务变长资源被锁定的问题，且协调者存在**单点故障**的问题；而且使用场景也是**针对数据层面**，只能在单服务多数据库这样；而微服务场景下，多服务多数据库情况下，各个微服务通过网络接口调用，间接访问对方服务的数据库就会被局限；
...
![image-20230918232556197](assets/image-20230918232556197.png)

##### **Seata-AT**(Automatic Transaction) 二阶段提交

Seata提供了AT、TC、SAGA、XA等事务模式；其中`AT是二阶段提交协议的一种落地实现`。
