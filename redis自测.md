...
- 有两个分别指向头尾节点的指针，加上节点的前后指针，使得双端列表支持从头尾两个方向的遍历，前后指针都可以指向null，所以链表是无环的；
- 保存头尾两个节点的指针，在链表的头/尾进行插入/查询的复杂度都为o(1)；
- 同时存储了链表长度字段，可以o(1)获取链表的长度;

ListNode每个节点存储着指向前后节点的指针；
